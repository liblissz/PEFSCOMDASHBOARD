...
const Sidebar = ({ sidebarHidden }) => {
  ...
  return (
    <section id="sidebar" className={sidebarHidden ? 'hide' : ''}>
      <a href="#" className="brand "  id='po'>
        <CardSimIcon size={24} /> 
        <span className={sidebarHidden?"textno":"text big"}>PEFSCOM SYSTEM</span>
      </a>
  ...
  <Link to={"/analytics"} style={{cursor: "pointer"}}>
     ...
     <Link to={'/orders'}>
        <li title='Orders'><a  id='po'><MessageCircle /><span className={sidebarHidden?"textno":"text"}>User Orders</span></a></li>
        </Link>
        <Link to={'/Team'}> 
