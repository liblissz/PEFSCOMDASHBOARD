// ✅ Full corrected version of AccountSettings.js with rating fix
...
const AccountSettings = () => {
  ...
  const [formData, setFormData] = useState({ username: '', email: '', number: '', about: '' });
    ...
    const fetchAll = async () => {
      try {
        ...
        projectRes.data.forEach(proj => {
          const myRating = proj.ratings.find(r => r.userId === profileRes.data._id);
          ratings[proj._id] = myRating ? myRating.value : 0;
        });
  ...
  const handleRatingChange = async (projectId, rating) => {
    try {
      ...
      const updatedProject = await axios.get(
        `https://pefscombackendprivate.onrender.com/api/user/project/${projectId}`
      );
      setprojectdetails(prev =>
        prev.map(p => p._id === projectId ? updatedProject.data : p)
      );
            ...
            <div className="project-card" key={item._id}>
              ...
              <p>
                Avg: {item.ratings?.length
                  ...
                  : "0.0"} ⭐ ({item.ratings?.length || 0} ratings)
              </p>
            </div>
          ))}
      </div>
...
//       // Optional: fetch user ratings for these projects or get from projectRes.data if included
//       // Assuming your backend sends rating info per user as 'userRating' in each project item
//       const ratings = {};
//       projectRes.data.forEach(proj => {
//         ratings[proj._id] = proj.userRating || 0; // 0 if not rated
//       });
//       setUserRatings(ratings);
//     } catch (error) {
//       toast.error("Error loading data");
